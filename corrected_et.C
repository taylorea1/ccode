...
constexpr const char *kCodeVersion = "2026-07-29-v7";
        ...
        "/home/amarit/rootdisplay/genrp_replayed_1071_20k_events.root")
{
    ...
    auto *eventCanvas = new TCanvas("eventCanvas", "Detector Event Display", 800, 600);
    eventCanvas->SetRightMargin(0.28);
    auto *detector =
        new TH2F("detector", "Detector;X;Y", 8, -0.5, 7.5, 4, -0.5, 3.5);
    detector->SetStats(false);

    // Legend samples are created once and used by a cloned legend on each
    // event page. Hit numbers on the plot give increasing TDC order; they are
    // deliberately not joined by a line because this is not a fitted track.
    auto *geometryExample = new TMarker(0, 0, 20);
    geometryExample->SetMarkerColor(kGray + 2);

    auto *earliestExample = new TMarker(0, 0, 20);
    earliestExample->SetMarkerColor(kCyan + 2);

    auto *intermediateExample = new TMarker(0, 0, 20);
    intermediateExample->SetMarkerColor(kGray + 1);

    auto *latestExample = new TMarker(0, 0, 20);
    latestExample->SetMarkerColor(kOrange + 7);

    auto *watchExample = new TMarker(0, 0, 29);
    watchExample->SetMarkerColor(kMagenta + 2);

    auto *previousExample = new TMarker(0, 0, 21);
    previousExample->SetMarkerColor(kBlue);

    auto *nextExample = new TMarker(0, 0, 22);
    nextExample->SetMarkerColor(kGreen + 2);

    auto *eventLegend = new TLegend(0.73, 0.53, 0.99, 0.92);
    eventLegend->SetHeader("Event-display key", "C");
    eventLegend->SetBorderSize(1);
    eventLegend->SetFillColor(kWhite);
    eventLegend->SetFillStyle(1001);
    eventLegend->SetTextSize(0.025);
    eventLegend->AddEntry(geometryExample, "Channel position", "p");
    eventLegend->AddEntry(earliestExample, "Earliest TDC hit", "p");
    eventLegend->AddEntry(intermediateExample, "Intermediate TDC hit", "p");
    eventLegend->AddEntry(latestExample, "Latest TDC hit", "p");
    eventLegend->AddEntry(watchExample, "Watched channel", "p");
    eventLegend->AddEntry(previousExample, "Previous in TDC order", "p");
    eventLegend->AddEntry(nextExample, "Next in TDC order", "p");
    eventLegend->AddEntry(
    ...
    auto *multiplicityCanvas =
        new TCanvas("multiplicityCanvas",
                    "Per-Event Channel Multiplicity", 1000, 600);
    multiplicityCanvas->SetRightMargin(0.15);

    auto *eventMultiplicity =
        new TH2I("eventMultiplicity",
                 "Per-Event Matched-Hit Multiplicity;"
                 "Detector X;Detector Y",
                 8, -0.5, 7.5, 4, -0.5, 3.5);
    eventMultiplicity->SetStats(false);
    eventMultiplicity->SetMinimum(0.0);
    eventMultiplicity->GetZaxis()->SetTitle("Matched ADC/TDC hits");

    const TString combinedPdf =
        Form("channel_%02d_events_and_PID.pdf", watchChannel);
    const TString multiplicityPdf =
        Form("channel_%02d_event_multiplicity.pdf", watchChannel);
    eventCanvas->Print(combinedPdf + "[");
    multiplicityCanvas->Print(multiplicityPdf + "[");

    Long64_t multiplicityPages = 0;

    // Second pass: fill output, draw hit-order displays, and save
    // watched-channel events.
    for (Long64_t event = 0; event < nEntries; ++event) {
        if (tree->GetEntry(event) <= 0) {
            continue;
        }
        ...
        // Count all matched ADC/TDC hits in every detector channel for this
        // saved watched-channel event. Repeated hits are kept by BuildHits().
        std::array<int, kNChannels> channelMultiplicity{};
        for (const Hit &eventHit : hits) {
            ++channelMultiplicity[eventHit.channel];
        }
        ...
        for (std::size_t i = 0; i < hits.size(); ++i) {
            ...
            auto *marker = new TMarker(p.x, p.y, 20);
            const Color_t hitColor =
                i == 0 ? kCyan + 2 :
                (i + 1 == hits.size() ? kOrange + 7 : kGray + 1);
            marker->SetMarkerColor(hitColor);
            marker->SetMarkerSize(2.0);
            marker->Draw();
            marker->SetBit(kCanDelete);

            auto *orderLabel =
                new TLatex(p.x - 0.20, p.y - 0.20, Form("#%zu", i + 1));
            orderLabel->SetTextColor(hitColor);
            orderLabel->SetTextSize(0.027);
            orderLabel->SetTextFont(62);
            orderLabel->Draw();
            orderLabel->SetBit(kCanDelete);
        }
        ...
        // Draw a fresh clone because eventCanvas->Clear() deletes objects
        // belonging to the preceding event page.
        eventLegend->DrawClone();
        ...
        // Make one 2D detector multiplicity map for this event. Each detector
        // cell is a channel; its color and N= label show the number of matched
        // ADC/TDC hits in that channel.
        multiplicityCanvas->cd();
        multiplicityCanvas->Clear();
        eventMultiplicity->Reset();
        eventMultiplicity->SetTitle(
            Form("Event %s: Matched-Hit Multiplicity;"
                 "Detector X;Detector Y",
                 EntryText(event).c_str()));

        int maximumMultiplicity = 1;
        for (int channel = 0; channel < kNChannels; ++channel) {
            const ChannelPosition &p = GetPosition(channel);
            const int xBin = eventMultiplicity->GetXaxis()->FindBin(p.x);
            const int yBin = eventMultiplicity->GetYaxis()->FindBin(p.y);
            const int multiplicity = channelMultiplicity[channel];

            eventMultiplicity->SetBinContent(xBin, yBin, multiplicity);
            maximumMultiplicity =
                std::max(maximumMultiplicity, multiplicity);
        }

        eventMultiplicity->SetMaximum(
            static_cast<double>(maximumMultiplicity));
        eventMultiplicity->Draw("COLZ");
        ...
        for (int channel = 0; channel < kNChannels; ++channel) {
            ...
            auto *channelLabel =
                new TLatex(p.x, p.y + 0.12, Form("Ch %d", channel));
            channelLabel->SetTextAlign(22);
            channelLabel->SetTextSize(0.025);
            channelLabel->SetTextColor(kBlack);
            channelLabel->Draw();
            channelLabel->SetBit(kCanDelete);

            auto *multiplicityLabel =
                new TLatex(p.x, p.y - 0.15, Form("N=%d", multiplicity));
            multiplicityLabel->SetTextAlign(22);
            multiplicityLabel->SetTextSize(0.032);
            multiplicityLabel->SetTextFont(62);
            multiplicityLabel->SetTextColor(
                multiplicity > 1 ? kRed + 1 : kBlack);
            multiplicityLabel->Draw();
            multiplicityLabel->SetBit(kCanDelete);
        }

        auto *multiplicityNote = new TLatex(
            0.72, 0.94, "Red N>1: multiple matched hits");
        multiplicityNote->SetNDC();
        multiplicityNote->SetTextColor(kRed + 1);
        multiplicityNote->SetTextSize(0.026);
        multiplicityNote->SetTextFont(62);
        multiplicityNote->Draw();
        multiplicityNote->SetBit(kCanDelete);

        multiplicityCanvas->Modified();
        multiplicityCanvas->Update();
        multiplicityCanvas->Print(multiplicityPdf);
        ++multiplicityPages;
    ...
    delete eventLegend;
    delete geometryExample;
    delete earliestExample;
    delete intermediateExample;
    delete latestExample;
    delete watchExample;
    delete previousExample;
    delete nextExample;
    ...
    std::cout << "\nAnalysis complete.\n"
              ...
              << combinedPdf << '\n'
              << "Saved " << EntryText(multiplicityPages)
              << " per-event 2D multiplicity maps: "
              << multiplicityPdf << '\n';
}
...
void eventracker_corrected(
    ...
    const char *inputFile =
        "/home/amarit/rootdisplay/genrp_replayed_1071_20k_events.root")
{
    corrected_et(watchChannel, inputFile);
}